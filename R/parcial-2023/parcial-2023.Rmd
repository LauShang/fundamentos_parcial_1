...
**Finalmente el estimador(proporción) para cada opción es**
...
**Primero obtenemos una lista con todos los estratos de la muestra, y una lista con los totales de casilla por estrato**

```{r}
#Tabla que usaremos para las remuestras
...
EST
#Totales de casilla por estrato
...
```
**La distribución está sesgada a la izquierda, ya que tiene colas cortas a la izquierda, y colas largas a la derecha **

4.  Reporta tus intervalos en una tabla. Compara la longitud de los 3 intervalos y describe que observas.

|                  |  Inferior| Superior | Longitud |  p estimado||  p real   |
|:-----------------|---------:|---------:|---------:|------------:|----------:|
|Opción Sí         | 0.9011299| 0.9607356|0.0596057 |0.9307454    |0.9772147  |
|Opción No         |0.01383786|0.01572887|0.00189101|0.01477969   |0.01544977 |
|Nulos             |0.02407799|0.08448975|0.06041176|0.05447491   |0.007335506|


**Los estimadores de P para cada una de las variables, si se encuentran dentro del intervalo.**


5.  ¿Tus intervalos contienen los valores observados en los cómputos? Explica los resultados observados.

**Sólo uno de los valores reales se encuentran dentro del intervalo calculado, el de la opción "No".**
...
**Construyo la tabla para seleccionar de ahí las muestras**
...
```{r}
intervalos_tot = tibble()

for (k in 1:50) {
  tabla_aux <- muestras_calibracion[which(muestras_calibracion$id == k),]
  tabla_aux %>% select(ESTRATO, SI, NO, NULOS, TOTAL,n)
  boot_aux <- Super_boot(tabla_aux, estratos, EST, tablaN)
  intervalo_aux <- boot_aux  %>% 
    summarize(si_int_inf = mean(p_SI)-2*sd(p_SI), si_int_sup = mean(p_SI)+2*sd(p_SI),
              no_int_inf = mean(p_NO)-2*sd(p_NO), no_int_sup = mean(p_NO)+2*sd(p_NO),
              nul_int_inf = mean(p_NUL)-2*sd(p_NUL), nul_int_sup = mean(p_NUL)+2*sd(p_NUL),
              si_int_infq = quantile(p_SI,0.025), si_int_supq = quantile(p_SI,0.975),
              no_int_infq = quantile(p_NO,0.025), no_int_supq = quantile(p_NO,0.975),
              nul_int_infq = quantile(p_NUL,0.025), nul_int_supq = quantile(p_NUL,0.975))
  intervalo_aux <- cbind(k,intervalo_aux)
  intervalos_tot <- rbind(intervalos_tot, intervalo_aux)
}
...
```{r}
prop_verdadera <- computos %>% 
  summarise(propSi_verdadera = sum(OPINION_SI)/sum(TOTAL_OPINIONES),
            propNo_verdadera = sum(OPINION_NO)/sum(TOTAL_OPINIONES),
            propNul_verdadera = sum(NULOS)/sum(TOTAL_OPINIONES))

propSi_verdadera <- prop_verdadera %>% pull(propSi_verdadera)
propNo_verdadera <- prop_verdadera %>% pull(propNo_verdadera)
propNul_verdadera <- prop_verdadera %>% pull(propNul_verdadera)
...
interval_cubre <- intervalos_tot |> 
#  bind_rows() |>
  mutate(cubre_sn = si_int_inf   < propSi_verdadera & propSi_verdadera < si_int_sup,
         cubre_sq = si_int_infq  < propSi_verdadera & propSi_verdadera < si_int_supq,
         cubre_nn = no_int_inf   < propNo_verdadera & propNo_verdadera < no_int_sup,
         cubre_nq = no_int_infq  < propNo_verdadera & propNo_verdadera < no_int_supq,
         cubre_un = nul_int_inf  < propNul_verdadera & propNul_verdadera < nul_int_sup,
         cubre_uq = nul_int_infq < propNul_verdadera & propNul_verdadera < nul_int_supq) 

norm_si <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propSi_verdadera, colour = "red") +
  geom_linerange(aes(ymin = si_int_inf, ymax = si_int_sup, colour = cubre_sn)) 

quart_si <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propSi_verdadera, colour = "red") +
  geom_linerange(aes(ymin = si_int_infq, ymax = si_int_supq, colour = cubre_sq)) 

norm_no <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propNo_verdadera, colour = "red") +
  geom_linerange(aes(ymin = no_int_inf, ymax = no_int_sup, colour = cubre_nn)) 

quart_no <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propNo_verdadera, colour = "red") +
  geom_linerange(aes(ymin = no_int_infq, ymax = no_int_supq, colour = cubre_nq)) 

norm_nul <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propNul_verdadera, colour = "red") +
  geom_linerange(aes(ymin = nul_int_inf, ymax = nul_int_sup, colour = cubre_un)) 

quart_nul <- ggplot(interval_cubre, aes(x = k)) +
  geom_hline(yintercept = propNul_verdadera, colour = "red") +
  geom_linerange(aes(ymin = nul_int_infq, ymax = nul_int_supq, colour = cubre_uq)) 
...
```{r}
interval_cubre %>%
  summarise(cobertura_SI_sd = mean(cubre_sn),
            cobertura_SI_qt = mean(cubre_sq),
            cobertura_NO_sd = mean(cubre_nn),
            cobertura_NO_qt = mean(cubre_nq),
            cobertura_NULOS_sd = mean(cubre_un),
            cobertura_NULOS_qt = mean(cubre_uq))
```
**Solo 88% de los intervalos de la Opción Sí contienen al valor real.**
**Solo 88% de los intervalos de la Opción No contienen al valor real**
**Solo 74% de los intervalos de los Nulos contienen al valor real**

**Estos porcentajes de cobertura menores al 95%, se deben a que la distribución, como observamos, se encuentra sesgada a la izquierda**
...
**Hay varios datos atípicos con votos NULOS arriba de 500, hay casillas donde son equiparables al total de votos**

```{r}
nul_tabla <- muestra_tidy %>% select(NULOS, TOTAL, ESTRATO)
...
nul_tabla2 <- nul_tabla %>% 
  filter(NULOS <= 500)
nul3 <- ggplot(nul_tabla2, aes(x = NULOS)) + geom_histogram(bins = 15, fill = "red")
...
```{r}
nul_tabla3 <- nul_tabla %>% mutate(prop = NULOS/TOTAL)

nul4 <- ggplot(nul_tabla3, aes(x = prop)) + geom_histogram(bins = 15, fill = "red")
...
```{r}
#y <- nul_tabla %>% select(TOTAL)
#x <- nul_tabla %>% select(NULOS)
group <- as.factor(ifelse(nul_tabla$NULOS > 500, "Atípico", "No atípico"))

plot(x=nul_tabla$TOTAL, y=nul_tabla$NULOS, pch = as.numeric(group), col = group )
...
**Probablemente se deben a casillas donde hubo disturbios (o eventualidades) y tuvieron que cerrarse antes de tiempo por seguridad de los representantes de casillas y asistentes. De ser esta suposición cierta,  se deberían de eliminar ya que no representan el caso ideal donde los ciudadanos pueden libremente expresar su voto.**
